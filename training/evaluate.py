...
def main(
    checkpoint: str, config: str, split: str, max_samples: int, max_tokens: int, temperature: float
) -> None:

    from training.inference import build_prompt_tokens, load_model
    ...
    for i, rec in enumerate(records):
        ...
        with torch.inference_mode():
            output = model.generate(tokens, max_new=max_tokens, temperature=temperature, top_k=50)
    ...
    for target in sorted(results_by_target):
        ...
        tbl.add_row(target, str(len(r["struct"])), f"{avg_s:.2%}", f"{avg_l:.2%}", f"{avg_n:.2%}")
