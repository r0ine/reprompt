...
def main(skip_data: bool, skip_tokenizer: bool, config: str,
         count: int, vocab_size: int) -> None:

    console.print("[bold]ClarifyGPT — Tam Egitim Pipeline[/bold]")

    if not skip_data:
        if not run_step("1/5 Sentetik veri uretimi",
                        [PY, "-m", "training.data.generate_synthetic",
                         "--count", str(count)]):
            return

        if not run_step("2/5 Veri bolme (train/val/test)",
                        [PY, "-m", "training.data.split"]):
            return
    else:
        console.print("\n  [dim]Veri uretimi atlandi[/dim]")

    if not skip_tokenizer:
        if not run_step("3/5 Tokenizer egitimi",
                        [PY, "-m", "training.tokenizer.train_tokenizer",
                         "--vocab-size", str(vocab_size)]):
            return
    else:
        console.print("\n  [dim]Tokenizer atlandi[/dim]")

    if not run_step("4/5 Model egitimi",
                    [PY, "-m", "training.train_scratch", "--config", config]):
        return
    ...
    if best_ckpt.exists():
        run_step("5/5 Degerlendirme",
                 [PY, "-m", "training.evaluate",
                  "--checkpoint", str(best_ckpt), "--config", config])
    else:
